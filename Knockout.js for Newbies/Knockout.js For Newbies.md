...
You can now start noticing how messy things are getting, we have view logic (logic that modifies the DOM) in your ```addTodoItem```, which is a function that is supposed to just add a TODO item, we are mixing logic, the bigger our app gets, the more we will mix things, and if we have to debug something, or modify something several months from now, it will be hard. For now, we'll finish the simple example with plain Javascript, and then, still with plain Javascript, try to make it more organized.